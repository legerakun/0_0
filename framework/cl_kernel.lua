...
function Clockwork:MenuItemsAdd(menuItems)
	...
	menuItems:Add("Настройки", "cwSettings", "Доступ к настройкам клиента.", cwOption:GetKey("icon_data_settings"));
					...
					elseif (trace.Entity:IsWeapon()) then
						if (cwClient:GetShootPos():Distance(trace.HitPos) <= fadeDistance) then
							...
							if (!active) then
								...
								y = cwKernel:DrawInfo("Нажмите, чтобы экипировать.", x, y, colorWhite, alpha);
							end;
						end;
