...
function Clockwork.player:DeleteCharacter(player, characterID)
	...
	if (character) then
		...
		return false, "Персонаж не существует!";
	end;
end;
...
function Clockwork.player:UseCharacter(player, characterID)
	...
	if (currentCharacter != character or isCharacterMenuReset) then
		...
		if (fault == nil or fault == true) then
			...
			if (limit and players == limit) then
				...
				if (currentCharacter) then
					...
					if (fault != nil and fault != true) then
						return false, fault or "Вы не можете изменить персонажа!";
					end;
				end;
			...
			return false, fault or "Вы не можете использовать персонажа!";
		end;
	else
	...
	if (cwCfg:Get("cash_enabled"):Get()) then
		...
		if (roundedAmount < 0) then
			...
			if (!bNoMsg) then
				if (reason) then
					cwHint:Send(
						player, "Вы потеряли "..cwKernel:FormatCash(roundedAmount).." ("..reason..").", 4, negativeHintColor
					);
				else
					cwHint:Send(
						player, "Вы потеряли "..cwKernel:FormatCash(roundedAmount)..".", 4, negativeHintColor
					);
				end;
			end;
		elseif (roundedAmount > 0) then
			if (!bNoMsg) then
				if (reason) then
					cwHint:Send(
						player, "Вы получили  "..cwKernel:FormatCash(roundedAmount).." ("..reason..").", 4, positiveHintColor
					);
				else
					cwHint:Send(
						player, "Вы получили "..cwKernel:FormatCash(roundedAmount)..".", 4, positiveHintColor
					);
				end;
			end;
		end;
		...
		if (!player.cwCharacterList[characterID]) then
			...
			if (character and type(character) == "table") then
				...
				if (!bForce) then
					...
					if (fault == false or type(fault) == "string") then
						return self:SetCreateFault(player, fault or "Вы не можете создать этого персонажа!");
					end;
				end;
