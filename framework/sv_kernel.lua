...
function Clockwork:PlayerInitialSpawn(player)
	...
	if (!player:IsKicked()) then
		...
		cwChatbox:Add(nil, nil, 'connect', player:SteamName()..' зашел на сервер.');
	end;
end;
...
function Clockwork:PlayerCanDeleteCharacter(player, character)
	if (cwConfig:Get("cash_enabled"):Get() and character.cash < cwConfig:Get("default_cash"):Get()) then
		if (!character.data["CharBanned"]) then
			return "Вы не можете удалить персонажа с меньшим кол-вом крышек чем "..cwKernel:FormatCash(cwConfig:Get("default_cash"):Get(), nil, true)..".";
		end;
	end;
end;
...
function Clockwork:PlayerCanSwitchCharacter(player, character)
	if (!player:Alive() and !player:IsCharacterMenuReset() and !player:GetSharedVar("CharBanned")) then
		return "Вы не можете сменить персонажа, когда вы мертвы!";
	elseif (player:GetRagdollState() == RAGDOLL_KNOCKEDOUT) then
		return "Вы не можете сменить персонажа, когда вы без сознания!";
	end;
		...
		if (attacker:IsPlayer()) then
			if (IsValid(attacker:GetActiveWeapon())) then
				...
				if (IsValid(weapon) and itemTable) then
					cwKernel:PrintLog(LOGTYPE_CRITICAL, attacker:Name().." нанес "..tostring(math.ceil(damageInfo:GetDamage())).." урона по "..player:Name().." с "..itemTable("name")..", убив его!");
				else
					cwKernel:PrintLog(LOGTYPE_CRITICAL, attacker:Name().." нанес "..tostring(math.ceil(damageInfo:GetDamage())).." урона по "..player:Name().." с "..cwPly:GetWeaponClass(attacker)..", убив его!");
				end;
			else
				cwKernel:PrintLog(LOGTYPE_CRITICAL, attacker:Name().." нанес "..tostring(math.ceil(damageInfo:GetDamage())).." урона по "..player:Name()..", убив его!");
			end;
		else
			cwKernel:PrintLog(LOGTYPE_CRITICAL, attacker:GetClass().." нанес "..tostring(math.ceil(damageInfo:GetDamage())).." урона по "..player:Name()..", убив его!");
		end;
	end;
end;
			...
			if (damageInfo:IsDamageType(DMG_CRUSH) and damageInfo:GetDamage() < 10) then
				...
				if (damageInfo:GetDamage() > 0) then
					...
					if (player:Alive() and player:Health() == 1) then
						...
						if (attacker:IsPlayer()) then
							cwKernel:PrintLog(LOGTYPE_MAJOR, player:Name().." принял "..tostring(math.ceil(damageInfo:GetDamage())).." урона от "..attacker:Name().." с "..cwPly:GetWeaponClass(attacker, "an unknown weapon")..", оставив ему "..player:Health().." "..armor);
						else
							cwKernel:PrintLog(LOGTYPE_MAJOR, player:Name().." принял "..tostring(math.ceil(damageInfo:GetDamage())).." урона от "..attacker:GetClass()..", оставив ему "..player:Health().." "..armor);
						end;
					end;
				end;
...
function playerMeta:GiveItem(itemTable, bForce)
	...
	and self:CanHoldSpace(itemTable("space"))) or bForce) then
		...
		cwKernel:PrintLog(LOGTYPE_GENERIC, self:Name().." получил "..itemTable("name").." "..itemTable("itemID")..".");
		...
		return false, "У вас нет места в инвентаре!";
	end;
end;
...
function playerMeta:TakeItem(itemTable)
	...
	cwKernel:PrintLog(LOGTYPE_GENERIC, self:Name().." потерял "..itemTable("name").." "..itemTable("itemID")..".");
...
concommand.Add("cwStatus", function(player, command, arguments)
	...
	if (IsValid(player)) then
		if (cwPly:IsAdmin(player)) then
			...
			player:PrintMessage(2, "У вас нет доступа к команде, "..player:Name()..".");
		end;
	else
		...
		if (text != "") then
			if (Clockwork.plugin:Call("PlayerCanSayOOC", player, text)) then
				if (!player.cwNextTalkOOC or curTime > player.cwNextTalkOOC or player:IsAdmin()) then
					...
					Clockwork.player:Notify(player, "Вы не можете говорить в ООС еще "..math.ceil(player.cwNextTalkOOC - CurTime()).." секунд(у)!");
					return;
				end;
			end;
		end;
