...
if (SERVER) then
	...
	function Clockwork.kernel:HandleEntityMenu(entity)
		...
		self:SetTitledMenu(menuPanel, "ДЕЙСТВИЯ");
	...
	function Clockwork.kernel:DrawDateTime()
		...
		if (Clockwork.LastDateTimeInfo and Clockwork.LastDateTimeInfo.y > info.y) then
			...
			self:DrawInfo("ПЕРСОНАЖ И РОЛЕВАЯ ИНФОРМАЦИЯ", x, y + 4, colorInfo, nil, true, function(x, y, width, height)
				return x, y - height;
			end);
			...
			if (self:CanCreateInfoMenuPanel() and self:IsInfoMenuOpen()) then
				...
				self:DrawInfo("ВЫБЕРИТЕ БЫСТРУЮ ФУНКЦИЮ", x, y, colorInfo, nil, true, function(x, y, width, height)
	...
	function Clockwork.kernel:SetRecogniseMenu(menuPanel)
		...
		self:SetTitledMenu(menuPanel, "КТО МОЖЕТ ВАС УЗНАТЬ");
	end;
