...
function PANEL:Init()
	if (!Clockwork.theme:Call("PreCharacterMenuInit", self)) then
		...
		self.authorLabel:SetText("РАЗРАБОТАНО "..string.upper(Schema:GetAuthor()));
		...
		self.createButton:SetText("НОВЫЙ");
		...
		self.createButton:SetCallback(function(panel)
			if (table.Count(Clockwork.character:GetAll()) >= Clockwork.player:GetMaximumCharacters()) then
				return Clockwork.character:SetFault("Вы не можете создать персонажа!");
			end;
...
function PANEL:Init()
	...
	self.useButton:SetToolTip("Использовать этого персонажа.");
...
function PANEL:Think()
	...
	markupObject:Add(
		self.customData.details or "Нету информации."
	);
...
function PANEL:Init()
	...
	self.helpText = self.attributesForm:Help("Вы можете потратить еще "..maximumPoints.." очко(в).");
...
function PANEL:Think()
	...
	if (self.helpText) then
		...
		self.helpText:SetText("Вы можете потратить еще "..pointsLeft.." очко(в).");
	end;
...
function PANEL:Init()
	...
	if (self.bSelectModel or self.bPhysDesc) then
		...
		if (self.bPhysDesc and self.bSelectModel) then
			self.appearanceForm:Help("Напишите описание, и выберите модель.");
		elseif (self.bPhysDesc) then
			self.appearanceForm:Help("Напишите описание на Английском языке!");
		end;
