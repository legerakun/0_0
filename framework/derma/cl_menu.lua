...
function PANEL:Rebuild(change)
	if (!Clockwork.theme:Call("PreMainMenuRebuild", self)) then
		...
		self.closeMenu:SetText(CW_CONVAR_CLOSESTRING:GetString() or "ЗАКРЫТЬ");
		self.closeMenu:SetCallback(function(button)
			self:SetOpen(false);
		end);
		self.closeMenu:SetToolTip("Нажмите, чтобы закрыть.");
		...
		self.characterMenu:SetText(CW_CONVAR_CHARSTRING:GetString() or "ПЕРСОНАЖИ");
		self.characterMenu:SetCallback(function(button)
			self:SetOpen(false);
			Clockwork.character:SetPanelOpen(true);
		end);
		self.characterMenu:SetToolTip("Нажмите, чтобы открыть меню персонажей.");
