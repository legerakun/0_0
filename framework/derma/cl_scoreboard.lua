...
function PANEL:Rebuild()
	...
	if (table.Count(classes) > 0) then
		local label = vgui.Create("cwInfoText", self);
			label:SetText("Нажмите на модель, для использования команд.");
		...
		local label = vgui.Create("cwInfoText", self);
			label:SetText("Здесь нет игроков.");
...
--	local descFont = Clockwork.fonts:GetSize(Clockwork.option:GetFont("scoreboard_desc"), 16);
	...
	if (info.avatarImage) then
		self.avatarButton:SetToolTip("Имя: "..info.steamName..".\nСтим ИД:"..info.player:SteamID()..".");
...
function PANEL:Think()
	if (IsValid(self.player)) then
		if (self.toolTip) then
			...
			self.spawnIcon:SetToolTip("Пинг: "..self.player:Ping()..".");
		end;
	end;
