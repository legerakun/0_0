...
if (CLIENT) then
	...
	SWEP.SlotPos = 5;
	SWEP.DrawAmmo = false;
	SWEP.PrintName = "Clever";
	SWEP.DrawCrosshair = true;
end

SWEP.Instructions = "Primary Fire: Slice";
SWEP.Purpose = "You can kill using this.";
SWEP.Contact = "";
...
SWEP.VElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/cleaver.mdl", bone = "v_weapon.knife_Parent", rel = "", pos = Vector(0.165, 1.25, 1.562), angle = Angle(0.048, -180, 90.249), size = Vector(1.001, 1.001, 1.001), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["element_name"] = { type = "Model", model = "models/Halokiller38/fallout/weapons/Melee/cleaver.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.801, 1.24, 2.197), angle = Angle(180, -90.002, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
	...
	if (SERVER) then
		...
		if (self.Owner:GetShootPos():Distance(trace.HitPos) <= 64) then
			if (IsValid(trace.Entity)) then
				...
				local strength = Clockwork.attributes:Fraction(self.Owner, ATB_MELWEP, 10, 5);
				...
				if (trace.Entity:IsPlayer() or trace.Entity:IsNPC()) then
					...
					self.Owner:ProgressAttribute(ATB_MELWEP, 1, true);
				else
					...
					if (IsValid(physicsObject)) then
						...
						if (!player) then
							...
							self.Owner:ProgressAttribute(ATB_MELWEP, 0.5, true);
						else
							timer.Simple(FrameTime() * 0.5, function()
								if (IsValid(trace.Entity)) then
									trace.Entity:TakeDamageInfo(Clockwork.kernel:FakeDamageInfo(self.Primary.Damage + strength, self, self.Owner, trace.HitPos, DMG_CLUB, 2));
